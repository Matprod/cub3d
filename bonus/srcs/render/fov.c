...
{
	if (game->fov.e2 > -fabs((game->fov.end.y / 64.0 * MINIMAP_TILE_SIZE)
			- game->fov.y0))
	{
		game->fov.err -= fabs((game->fov.end.y / 64.0 * MINIMAP_TILE_SIZE)
				- game->fov.y0);
...
void	set_fov(t_game *game, int len, t_vector direction, t_vector start)
{
	...
	game->fov.err = abs((int)(game->fov.end.x / 64.0 * MINIMAP_TILE_SIZE)
			- game->fov.x0)
		- fabs((game->fov.end.y / 64.0 * MINIMAP_TILE_SIZE) - game->fov.y0);
}
