...
/*   Updated: 2024/12/10 18:00:41 by Matprod          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_vector line_pos, float half_width)
{
	...
	camera_plane = vec_scalar_mult(game->player.direction, game->player.direction_adjust);
	right = vec_normalize(vec_rotate(game->player.direction, 90));
	ray_dir = vec_normalize(vec_sum(camera_plane, vec_scalar_mult(right, screen_ratio * half_width)));
	*collision = cast_two_d_ray(game, ray_dir);
	collision->distance = (collision->distance / 64) * cosf(vec_angle(ray_dir, game->player.direction));
}
